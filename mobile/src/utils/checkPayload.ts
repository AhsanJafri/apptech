...
/** Firestore documents must stay under 1 MiB — large ads.txt files can have 10k+ sellers. */
...
/** Trim check result before offline cache — same limits as Firestore. */
export function trimCheckForCache(result: CheckResult): CheckResult {
  return {
    ...result,
    sellers: [],
    issues: result.issues.slice(0, MAX_ISSUES_STORED),
    files: (result.files ?? []).map((file) => ({
      ...file,
      sellers: [],
      issues: file.issues.slice(0, MAX_ISSUES_STORED),
      variables: file.variables?.slice(0, 20),
      trackedLineResults: file.trackedLineResults,
    })),
  };
}
